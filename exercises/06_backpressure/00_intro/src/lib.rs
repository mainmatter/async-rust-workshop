...
//!
//! Nothing to write here. One question: what should happen when requests arrive faster than the
//! store can apply them?
//!
//! There are only three answers. Queue them, and hope. Make the sender wait. Or say no.
//!
//! **An unbounded queue is the answer people reach for by accident.** It never blocks, so it never
//! shows up in a profile, and under sustained overload it grows until the process is killed. The
//! latency goes first: a request that sits in a queue for thirty seconds is a request whose client
//! gave up twenty-eight seconds ago, and the work is done anyway. `mpsc::unbounded_channel` exists
...
//!
//! **Saying no is the third answer**, and it is the next exercise. Waiting is only useful if
//! somebody is still there to wait.
