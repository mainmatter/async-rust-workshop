...
mod tests {
    ...
    async fn a_full_mailbox_makes_the_sender_wait() {
        ...
        let senders = (0..3)
            .map(|i| {
                ...
                tokio::spawn(async move { store.apply(set(&format!("user-{i}"))).await })
            })
            .collect::<Vec<_>>();
        ...
        assert!(
            started.elapsed() >= SLOW * 3,
            "three requests, one at a time, at {SLOW:?} each"
        );
    }

    #[tokio::test(start_paused = true)]
    async fn nothing_is_lost_while_waiting() {
        let store = StoreHandle::spawn_with_capacity(Store::new(), SLOW, 1);

        for i in 0..5 {
            assert_eq!(store.apply(set(&format!("user-{i}"))).await, Response::Ok);
        }
