//! # Exercise
//!
//! The mutex works, and it has a shape worth noticing: every connection reaches into shared state
//! and takes a turn. Nothing in any signature says how long a turn lasts, or that the store is
//! shared at all. `Arc<Mutex<T>>` is not a design, it is the absence of one.
//!
//! The alternative is to give the store to a single task and talk to it. Nobody locks anything,
//! because nobody else can touch it: `Store` is owned by `run`, exclusively, for its whole life.
//!
//! Implement the two `todo!()`s in `actor.rs`:
//!
//! - `run` owns the store, takes commands off its inbox one at a time, applies them with `apply`
//!   from `server.rs`, and sends each answer back down the command's own reply channel. When every
//!   handle has been dropped, `recv` returns `None` and the task is done.
//! - `StoreHandle::apply` builds a `oneshot` pair, sends the command, and awaits the reply.
//!
//! Then rewrite `handle_connection` in `server.rs` to use the handle instead of the mutex. It gets
