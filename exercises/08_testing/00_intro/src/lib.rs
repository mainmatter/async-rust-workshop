//! # Chapter 8: testing what only happens sometimes
//!
//! There is nothing to write in this exercise. Read `tests/harness.rs`, run `wr`, and it passes.
//!
//! Async code is hard to test for three reasons, and Tokio has an answer to each of them.
//!
//! **It takes time.** The idle timeout you wrote in chapter 5 fires after thirty seconds, and no
//! test suite can afford to wait. `#[tokio::test(start_paused = true)]` replaces the clock with one
//! the runtime controls: when every task is waiting on a timer, the runtime jumps straight to the
//! next deadline. Thirty seconds pass in microseconds, and `Instant::now` agrees they did.
//!
//! **It needs a peer.** A server wants a socket, and a socket wants a port, and a fixed port makes
//! the suite fail when two tests run at once. Bind `127.0.0.1:0` and ask the listener what it got.
//! Better still, do not use a socket: `tokio::io::duplex` is a pair of connected pipes that
//! implement `AsyncRead` and `AsyncWrite`, so you can hand one half to `handle_connection` and keep
//! the other, and control the interleaving byte by byte.
//!
//! **It is concurrent.** Which is to say the failure you are chasing happens once in fifty runs on
//! your machine and every time on someone else's. Most of the fix is in the design, not the test:
//! one task owning the store is testable in a way that a lock held across an await is not.
//!
//! The tests here demonstrate all three, plus the one thing about a paused clock that surprises
