//! # Exercise
//!
//! The store task is the one task in `minidb` that nothing can replace. If it dies, every handle to
//! it starts answering `ERR the store is gone`, forever, and the server keeps accepting connections
//! so it can tell them so. A process that is up but cannot do anything is worse than one that is
...
//! is usually the right one for a process whose state was in the task that just died.
