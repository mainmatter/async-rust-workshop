//! # Exercise
//!
//! The store task applies one request at a time, so a slow request delays every client behind it,
//! and a client waiting on a reply has no way to give up.
//!
//! Bound it. In `handle_connection`, give each request at most `REQUEST_LIMIT` to be answered, and
//! reply `ERR busy` when it takes longer. `tokio::time::timeout` returns `Err(Elapsed)` and drops
//! the request future.
//!
//! Then read the third test, which is the honest half of this exercise: the request that timed out
//! was still applied. The store task never learned that anybody had stopped waiting, so it did the
//! work and sent the answer into a channel with nobody at the other end.
//!
