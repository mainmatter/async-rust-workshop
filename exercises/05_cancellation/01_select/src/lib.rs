//! # Exercise
//!
//! A connection that says nothing holds a task, a socket, and a file descriptor for as long as the
//! client feels like it, and a client that has quietly gone away says nothing forever. Servers that
//! do not time out idle connections run out of file descriptors on a Tuesday.
//!
//! Give `handle_connection` an idle timeout. It already takes the limit as an argument and ignores
//! it; the loop should wait for either the next request or the deadline, whichever comes first, and
//! return when the deadline wins.
//!
//! `tokio::select!` is the tool: it polls several futures and takes the first one that is ready,
//! dropping the rest. The branch you do not take is cancelled, which is the entire subject of this
//! chapter and the reason the next exercise exists.
//!
//! Note what the timeout is measured from. Each trip round the loop starts a fresh `sleep`, so the
//! limit is time since the last request, not time since the connection opened. A client that says
//! something every few seconds should be left alone forever.
